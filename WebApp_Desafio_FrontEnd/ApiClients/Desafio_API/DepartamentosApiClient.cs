...
namespace WebApp_Desafio_FrontEnd.ApiClients.Desafio_API
{
    public class DepartamentosApiClient : BaseClient
    {
        ...
        private const string departamentoExcluirUrl = "api/Departamentos/Excluir";
        ...
        public bool CriarDepartamento(DepartamentoViewModel departamento)
        {
            var headers = new Dictionary<string, object>()
            {
                { "TokenAutenticacao", tokenAutenticacao }
        ...
        public bool DepartamentoExcluir(int idDepartamento)
        {
            ...
            var response = base.Delete($"{desafioApiUrl}{departamentoExcluirUrl}/{idDepartamento}", headers);
